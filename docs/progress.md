...
- **2026-04-19**
  - ダッシュボードのレイアウトを、カレンダー・サマリー・履歴リストが縦に並ぶ「家計簿スタイル」に刷新。
  - 複数の食材を消費して新しい料理（在庫）を作成する「料理（在庫変換）」機能を実装。原価も自動計算。
  - 消費および料理の記録時に、日付を自由に選択できる機能を追加。
  - 在庫管理専用ページ (`/inventory`) を作成。削除機能を実装。
